...
TxSigner.prototype.signFromMasterKey = function(masterseed, progress){
    ...
    var outputs = [];
    var isComplete = true;
    return task.seq(prepareInput(this.inputs, makeDerive(masterseed, this.split_path, this.network)).map(function(f){
        return function(){
            return f().then(function(sign){
                if(progress) progress(sign.idx + 1, input_max, sign);
                ...
                outputs.push({
                    redeemScript : sign.redeemScript.toString('hex'),
